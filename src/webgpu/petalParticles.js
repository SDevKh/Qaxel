...
export default class PetalParticles {
  ...
  createPetalMaterial(perlinTexture) {
    ...
    // Helper functions need to be uniquely named or scope-contained to avoid conflict
    function rotZ(a) {
      const c = cos(a);
      const s = sin(a);
      const ns = s.mul(-1.0);
  ...
  destroy() {
    if (this.#petalMesh) {
      ...
      if (Array.isArray(this.#petalMesh.material)) {
        this.#petalMesh.material.forEach((m) => m.dispose());
      } else {
