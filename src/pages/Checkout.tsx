    ...
    try {
      ...
      try {
        orderRef = await addDoc(collection(db, 'orders'), {
          ...formData, userId: user?.uid || 'anonymous', userEmail: user?.email || formData.email,
          createdAt: serverTimestamp(), total, subtotal, shipping, items, status: 'pending'
        });
      } catch (e) {
        try {
          const raw = localStorage.getItem('orders') || '[]';
          const arr = JSON.parse(raw);
          const localOrder = { id: `local-${Date.now()}`, ...formData, userId: user?.uid || 'anonymous', userEmail: user?.email || formData.email, createdAt: new Date().toISOString(), total, subtotal, shipping, items, status: 'pending' };
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-2xl shadow-md border border-[#F5C6D0]/20">
            ...
            <form className="space-y-5">
              <div>
                ...
                <input type="email" id="email" name="email" value={formData.email} onChange={handleInputChange} className={inputClass} />
              </div>
              ...
              {(Array.isArray(cartItems) ? cartItems : []).map((item: any) => (
                <div key={item.id} className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-medium text-[#4A2C3D]">{item.name}</h3>
                    <p className="text-xs text-[#8B5E6B]">Qty: {item.quantity}</p>
                  </div>
